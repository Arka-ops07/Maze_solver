...
export default function MazeVisualizer({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      ctx.font = 'bold 12px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('GOAL', 8 * CELL_SIZE, 8 * CELL_SIZE + 3);
      ...
      ctx.fillText(`${gearText}`, botState.pixelX + 14, botState.pixelY - 12);
  ...
  return (
    <div className="flex flex-col items-center bg-[#0a0a0a]/90 border border-[#222222] rounded p-2.5 shadow-none w-full">
      <div className="flex items-center justify-between w-full border-b border-[#111111] pb-1.5 mb-2">
        <h4 className="font-mono text-xs md:text-sm font-bold text-[#00ff88] uppercase tracking-wider">
          {title}
        </h4>
        <div className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-[#00ff88] shadow-[0_0_6px_#00ff88] animate-pulse"></span>
          <span className="font-mono text-[11px] text-neutral-550 uppercase tracking-tight font-medium">Interactive Screen</span>
        </div>
      </div>
